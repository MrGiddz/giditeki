...
const testimonials = [
  {
    ...
    hint: 'woman portrait',
    quote:
      'KrestCore Technologies transformed our operations. Their expertise in cloud solutions was a game-changer for our business, improving our efficiency by over 40%.',
  },
  {
    name: 'Michael Chen',
    title: 'CTO, TechGurus',
...
const startupBenefits = [
    {
        ...
        description: 'We help you launch your Minimum Viable Product quickly to test your ideas and gather crucial user feedback.'
    },
    {
        icon: Scaling,
        title: 'Scalable Architecture',
        description: 'Building a solid foundation is key. We design and implement scalable cloud infrastructure that grows with your business.'
    },
    {
        icon: Layers,
        title: 'Dedicated Teams',
        description: 'Get access to our expert developers, designers, and project managers who become a seamless extension of your team.'
    }
]
    ...
    <div className="flex flex-col">
            ...
            {services.map((service) => (
              <motion.div key={service.title} className="h-full" variants={itemVariants}>
                <Link href={`/services/${service.slug}`} className="group h-full">
                  <Card className="flex h-full flex-col glassmorphic glow-border">
                    <CardHeader className="flex flex-row items-center gap-4 p-6">
                      <div className="rounded-lg bg-accent p-3">
                        <service.icon className="h-8 w-8 text-accent-foreground" />
                      </div>
                      <CardTitle className="text-xl group-hover:text-accent">{service.title}</CardTitle>
                    </CardHeader>
      ...
      >
        <div className="container mx-auto px-4">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-center text-lg text-muted-foreground">
            We are proud of the work we do. Here are some of our recent
            success stories.
          </p>
          <motion.div
          ...
          >
            {projects.map((project) => (
              <motion.div key={project.title} variants={itemVariants}>
                <Link href={`/portfolio/${project.slug}`} className="group h-full block">
                  <Card className="overflow-hidden glassmorphic glow-border h-full">
                    <Image
                      src={project.image}
                      alt={project.title}
                      width={600}
      ...
      >
        <div className="container mx-auto px-4">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-center text-lg text-muted-foreground">
            We partner with ambitious startups, providing the technical firepower to turn innovative ideas into market-leading products.
          </p>
          <motion.div
          ...
          >
            {startupBenefits.map((benefit) => (
              <motion.div key={benefit.title} className="h-full" variants={itemVariants}>
                <Link href="/startups" className="group h-full">
                <Card className="flex h-full flex-col text-center glassmorphic glow-border">
